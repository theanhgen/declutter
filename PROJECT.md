...
| D10 | Per-site choice **refuse / accept / leave** (popup segmented control; settings lists). Plus Consent-O-Matic's **per-category choice** (A/B/D/E/F/X, settings "your choice"): all off = refuse, all on = accept, a mix is applied through the CMP's own API by `consent/categories.js` (Cookiebot, Didomi, OneTrust UI, cookieconsent v2/v3); any other CMP falls back to refuse-all and the popup says so | **built** 2026-09-24, user asked for CoM's options | chat |
